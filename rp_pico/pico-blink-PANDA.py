...
def main():
    ...
    panda = avatar.add_target(PandaTarget, gdb_port=1236,
    ...
    # 320KB of Flash (enough for blink.elf which is 287KB), Accessing using XIP with cache region
    # Note: This could be also mapped to the firmware file for performance
    xip_cached = avatar.add_memory_range(
        0x10000000, 0x00050000, name='Flash (XIP) - cache')
    ...
    print("Reached breakpoint")
    # Printing the state (xpsr/cpsr) register to validate state transfer
    reg_xpsr = pico.read_register("xpsr")[0]
    print(
        f"Read xpsr reg from pico before transfer 0x{reg_xpsr:x} ({reg_xpsr})")

    # PANDA does not have an xpsr register, so we use the cpsr register
    reg_cpsr = panda.read_register("cpsr")
    print(
        f"Read cpsr reg from panda before transfer 0x{reg_cpsr:x} ({reg_cpsr})")
    ...
    avatar.transfer_state(pico, panda, sync_regs=True,
                          synced_ranges=[ram, xip_cached])
    print("-------------- Transfer successful ---------------")

    reg_cpsr = panda.read_register("cpsr")
    print(
        f"Read cpsr reg from panda after transfer 0x{reg_cpsr:x} ({reg_cpsr})")
    ...
    panda.set_breakpoint(0x10001400)
    # Execute the emulation and wait for the breakpoint to get triggered
    panda.cont()
    panda.wait()
    IPython.embed()
    # Executing `panda.cont(); panda.wait()` in the ipython shell, will cause the LED
